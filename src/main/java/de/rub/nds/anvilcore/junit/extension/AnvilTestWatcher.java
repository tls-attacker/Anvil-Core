...
public class AnvilTestWatcher implements TestWatcher {
    ...
    private AnvilTestStateContainer createResult(ExtensionContext extensionContext, TestResult testResult) {
        String uniqueId = Utils.getTemplateContainerExtensionContext(extensionContext).getUniqueId();
        AnvilTestStateContainer testStateContainer = AnvilContext.getInstance().getTestResult(uniqueId);
    ...
    public synchronized void testSuccessful(ExtensionContext extensionContext) {
        ...
        AnvilTestStateContainer testStateContainer = createResult(extensionContext, TestResult.SUCCEEDED);

        if (!Utils.extensionContextIsBasedOnCombinatorialTesting(extensionContext.getParent().get())) {
    ...
    public synchronized void testFailed(ExtensionContext extensionContext, Throwable cause) {
        ...
        if (!(cause instanceof AssertionError)) {
            LOGGER.error("Test failed without AssertionError {}\n", extensionContext.getDisplayName(), cause);
        }

        String uniqueId = Utils.getTemplateContainerExtensionContext(extensionContext).getUniqueId();
        AnvilTestStateContainer testStateContainer = createResult(extensionContext, TestResult.FAILED);
        if (testStateContainer == null) {
            if (AnvilContext.getInstance().testIsFinished(uniqueId)) {
                return;
            }
            LOGGER.error("Illegal state: AnnotatedStateContainer is null even though test is not finished yet");
        }

        AnvilTestState testState = testStateContainer.getStates().stream()
                .filter(state -> state.getExtensionContext().getUniqueId().equals(extensionContext.getUniqueId()))
                .findFirst()
                .orElse(null);
    ...
    public void testDisabled(ExtensionContext extensionContext, Optional<String> reason) {
        ...
        AnvilTestStateContainer testStateContainer = createResult(extensionContext, TestResult.DISABLED);
